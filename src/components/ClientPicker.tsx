...
/** Müvekkil listesinden seçim yapan açılır alan (icra formu: alacaklı seçimi). */
export function ClientPicker({ label, value, onChange }: ClientPickerProps) {
  const __t = useTheme();
  ...
  return (
    <View style={styles.wrap}>
      <Text style={styles.label}>{label ?? t('enf.creditor')}</Text>
      <Pressable style={[styles.field, !selected && styles.fieldEmpty]} onPress={() => setOpen((o) => !o)}>
        ...
        <Text style={[styles.fieldText, !selected && styles.fieldPlaceholder]} numberOfLines={1}>
          {selected ? selected.full_name : t('enf.creditorPlaceholder')}
        </Text>
          ...
          {(clients ?? []).map((c) => (
            ...
            >
              <View style={styles.itemBody}>
                ...
                {!!c.company && <Text style={styles.itemSub}>{c.company}</Text>}
              </View>
