...
export interface GoruntuOlcusu { en: number; boy: number; tur: 'png' | 'jpeg' }
/**
 ...
 */
export function goruntuBoyutu(bayt: Uint8Array | null | undefined): GoruntuOlcusu | null {
  ...
  if (bayt[0] === 0x89 && bayt[1] === 0x50 && bayt[2] === 0x4E && bayt[3] === 0x47) {
    const g = (i: number) => (bayt[i] << 24 | bayt[i + 1] << 16 | bayt[i + 2] << 8 | bayt[i + 3]) >>> 0;
...
export function goruntuTuru(bayt: Uint8Array): string {
  const b = goruntuBoyutu(bayt);
  return b ? (b.tur === 'png' ? 'image/png' : 'image/jpeg') : 'application/octet-stream';
}

/**
 * Fotoğrafı küçültür ve JPEG'e çevirir.
 *
 * NEDEN ŞART: telefon kamerası 4000x3000 üretir. 30 hatalı bir denetimde bu,
 * 150 MB'lık bir Excel ve dolmuş bir cihaz deposu demektir. 1600 px uzun kenar
 * bir denetim fotoğrafı için fazlasıyla yeterli ve dosyayı ~40 kat küçültür.
 *
 * @param uri kameradan/galeriden gelen dosya adresi
 * @returns küçültülmüş dosyanın adresi ve piksel ölçüsü
 */
export async function fotografKucult(
  uri: string,
  enFazla = 1600,
  kalite = 0.82,
): Promise<{ uri: string; en: number; boy: number }> {
  const { ImageManipulator, SaveFormat } = await import('expo-image-manipulator');
  const baglam = ImageManipulator.manipulate(uri);
  baglam.resize({ width: enFazla });
  const islenmis = await baglam.renderAsync();
  const sonuc = await islenmis.saveAsync({ compress: kalite, format: SaveFormat.JPEG });
  return { uri: sonuc.uri, en: sonuc.width, boy: sonuc.height };
}
