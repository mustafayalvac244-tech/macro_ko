...
# --- roughness: polished; black mineral a touch less glossy ---
rough = 0.30 + 0.05 * mottle + 0.07 * dark + 0.03 * fine
rough = np.clip(rough, 0.27, 0.44)
Image.fromarray((rough * 255).astype(np.uint8)).save(
    os.path.join(TEX, "stone_roughness.png"))

# --- normal: gentle relief (blotches slightly sunken), micro speckle ---
height = nrm(0.5 * mottle + 0.1 * fine - 0.35 * dark)
...
Image.fromarray(((normal * 0.5 + 0.5) * 255).astype(np.uint8)).save(
    os.path.join(TEX, "stone_normal.png"))

print("wrote photo-matched serpentine maps ->", TEX)
