-- HASAT DARBOĞAZ ÖLÇÜMÜ — teorik hız ile gerçek hız neden ayrışıyor?
--
-- SORU (12.09.2026): 0094 hasadı "günde ~14.400 karar" olarak ayarladı
-- (3 kaynak × saatte 20 tur × 10 karar). ÖLÇÜLEN gerçek hız ise son 24 saatte
-- 4.769 karar — teoriğin yaklaşık üçte biri. Aradaki farkın sebebi BİLİNMİYOR:
--   • mükerrer karar (zaten havuzda) mı eleniyor,
--   • arama terimleri tükeniyor mu,
--   • kaynak hız sınırı (429) mı vuruyor,
--   • yoksa tur başına 10'luk tavana hiç ulaşılamıyor mu?
-- Hangisi olduğunu bilmeden hız ayarını yükseltmek işe yaramaz; kaynak KAMU
-- hizmeti ve IP yasağı özelliği tamamen öldürür (bkz. 0094'teki gerekçe).
--
-- Bu rapor scripts/hasat-tek-rapor.sql'den alındı; orada durduğu sürece
-- canlıda hiç koşturulamıyordu (migration-uygula yalnız migrations/ altını
-- çalıştırıyor). Hiçbir şeyi DEĞİŞTİRMEZ, yalnız okur.
-- ===========================================================================
-- (özgün başlık) HASAT TEK RAPOR — hepsi TEK sonuç tablosunda.
-- ===========================================================================
