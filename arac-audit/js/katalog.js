...
const YUZEY_MONTAJ = ['yuzey', 'montaj'];
const DIS_PANEL = ['yuzey', 'montaj', 'ses', 'sizdirmazlik', 'temizlik'];
const IC_PANEL = ['montaj', 'ses', 'yuzey', 'doseme', 'temizlik'];
const CAM_TAKIMI = ['cam', 'montaj', 'sizdirmazlik', 'ses', 'temizlik'];
const ELEKTRIK = ['fonksiyon', 'montaj', 'ses'];
...
function ciftle(parcalar) {
  const cikti = [];
  for (const p of parcalar) {
    if (!p.cift) { cikti.push(p); continue; }
    for (const [tk, tad, ten] of [['sol', 'Sol', 'LH'], ['sag', 'Sağ', 'RH']]) {
      cikti.push({
        ...p,
        cift: false,
        taraf: tk,
...
function yanBolge(taraf, ad, en, simge) {
  return {
...
export function parcaHataTipleri(parcaId) {
...
export function bolgelerAracIcin(arac) {
