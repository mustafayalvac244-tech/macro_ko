...
import { zipOlustur } from './zip.js';
import { goruntuBoyutu } from './goruntu.js';
...
export function hucreAdi(satir, sutun) { return `${sutunAdi(sutun)}${satir + 1}`; }
...
function hucreXml(satirNo, sutunNo, hucre) {
 ...
 */
function sayfaXml(sayfa, cizimVar) {
  ...
  const birlesik = (sayfa.birlesikler ?? []).length
    ? `<mergeCells count="${sayfa.birlesikler.length}">${sayfa.birlesikler.map((m) => `<mergeCell ref="${m}"/>`).join('')}</mergeCells>`
    : '';
...
export function sayfaAdiTemizle(ad) {
 ...
 */
export function gosterimOlcusu(bayt, enFazlaEn = 150, enFazlaBoy = 110) {
 ...
 * @returns {Uint8Array}
 */
export function xlsxOlustur(sayfalar, ustVeri = {}) {
  const tarih = ustVeri.tarih ?? new Date();
  const dosyalar = [];
  const icerikTurleri = [];
  const kitapSayfalari = [];
  const kitapIliski = [];
...
export function pikselPunto(piksel) { return Math.round((piksel / PUAN_PIKSEL) * 10) / 10; }
